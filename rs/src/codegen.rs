    ...
    for ep in endpoints {
        ...
        if ep.method == "get" || ep.method == "delete" {
            if ep.returns == "unit" {
                ...
                out += &format!("        return (await res.Content.ReadFromJsonAsync<{}>())!;\n", wrap_cs_type(&ep.returns));
            }
        } else {
            let body_var_name = body.map_or("body".to_string(), |b| camel(&b.wit_type));
            let body_var = body_var_name.as_str();
            if ep.returns == "unit" {
                out += &format!("        await http.{http_method}({url}, JsonContent.Create({body_var}));\n");
            } else {
                out += &format!("        var res = await http.{http_method}({url}, JsonContent.Create({body_var}));\n");
                out += &format!("        return (await res.Content.ReadFromJsonAsync<{}>())!;\n", wrap_cs_type(&ep.returns));
            }
        }
        out += "    }\n\n";
